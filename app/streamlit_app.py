...
def load_models() -> Dict[str, Any]:
    """Load all saved model artefacts. Missing files are handled gracefully."""
    import joblib
    models: Dict[str, Any] = {}
    artefacts = {
         # Regression
    "regression_model": MODELS_DIR / "rating_model.pkl",
    "regression_preprocessor": MODELS_DIR / "rating_preprocessor.pkl",

    # Classification
    "classification_model": MODELS_DIR / "visit_mode_model.pkl",
    "classification_preprocessor": MODELS_DIR / "visit_mode_preprocessor.pkl",
    ...
    for key, path in artefacts.items():
        if path.exists():
            try:
                ...
                models[key] = None
                st.sidebar.warning(f"⚠️ Could not load `{path.name}`: {e}")
        else:
            models[key] = None
    return models
...
def _build_input_row(
    ...
    Uses the globally loaded processed dataset (GLOBAL_DF) as a template
    so all columns expected by the trained models are present.
    """

    # Access globally loaded dataset
    global GLOBAL_DF

    # Start with one real row from the processed dataset
    if GLOBAL_DF is not None and not GLOBAL_DF.empty:
        row_df = GLOBAL_DF.iloc[[0]].copy()
    else:
        row_df = pd.DataFrame([{}])

    # Replace selected columns with user inputs
    mapping = {
        "Continent": continent,
        "Region": region,
        "Country": country,
...
def predict_rating(models: Dict, row_df: pd.DataFrame) -> Optional[float]:
    ...
    try:
        ...
        # rating_model.pkl is already a complete pipeline
        pred = model.predict(row_df)

        # Keep prediction within valid rating range
        return float(np.clip(pred[0], 1, 5))
...
def predict_visit_mode(
    ...
    """
    Predict visit mode and return class probabilities.
    """

    model = models.get("classification_model")
    preprocessor = models.get("classification_preprocessor")
    label_encoder = models.get("visit_mode_label_encoder")

    try:
        # Apply preprocessing if available
        X = preprocessor.transform(row_df) if preprocessor else row_df

        # Predict encoded class (e.g. 0,1,2,3,4)
        pred_class = safe_predict(model, X)[0]
        ...
        # Decode numeric class to original label
...
def shared_input_form(df: Optional[pd.DataFrame]) -> Dict[str, Any]:
    ...
    This version automatically converts numeric VisitMode values
    (1, 2, 3, 4, 5) into readable labels using the saved
    ...
    # Convert numeric values to labels if label encoder exists
    try:
        label_encoder = None
        ...
        if label_encoder is not None and len(visit_modes) > 0:
            # Check if values are numeric
            if all(str(v).replace(".", "").isdigit() for v in visit_modes):
                numeric_values = [int(float(v)) for v in visit_modes]

                visit_modes = list(
                    label_encoder.inverse_transform(numeric_values)
                )
    ...
    return dict(
        continent=continent, region=region, country=country, city=city,
        attraction_type=attraction_type, visit_mode=visit_mode,
